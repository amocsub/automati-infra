...
#   members = [
#     "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.automati-wip.name}/attribute.repository_owner/${var.github_org}",
#   ]
...
resource "google_service_account_iam_binding" "automati-k8s-role-binding" {
  service_account_id = google_service_account.automati-k8s-sa.id
  role               = "roles/iam.workloadIdentityUser"
  members = [
    "serviceAccount:${var.project_id}.svc.id.goog[actions-runner-system/automati-k8s-sa]",
  ]
  depends_on = [google_service_account.automati-k8s-sa]
}
